...
contract ShortsTrackerTest is Test {
    ...
    // Setter Tests //
    //////////////////
    function testShortsTrackerSetHandler() public {
        vm.prank(OWNER);
        shortsTracker.setHandler(USER, true);
        assertEq(shortsTracker.isHandler(USER), true);
    }

    function testShortsTrackerSetIsGlobalShortDataReady() public {
        vm.prank(OWNER);
        shortsTracker.setIsGlobalShortDataReady(true);
        assertEq(shortsTracker.isGlobalShortDataReady(), true);
    }

    event GlobalShortDataUpdated(address indexed token, uint256 globalShortSize, uint256 globalShortAveragePrice);

    function testShortsTrackerUpdateGlobalShortData() public {
        vm.startPrank(OWNER);
        shortsTracker.setHandler(OWNER, true);
        shortsTracker.setIsGlobalShortDataReady(true);
        shortsTracker.updateGlobalShortData(
            OWNER, weth, weth, true, 35689554309499200000000000000000000, 1000000000000000000000000000000000, true
        );
    }

    function testShortsTrackerSetInitData() public {
        address[] memory _tokens = tokenArray;
        numberArray.push(1000000000000000000000000000000000);
        numberArray.push(2000000000000000000000000000000000);
        uint256[] memory _averagePrices = numberArray;
        vm.prank(OWNER);
    ...
    function testShortsTrackerGetGlobalShortDelta() public {
        (bool hasProfit, uint256 delta) = shortsTracker.getGlobalShortDelta(weth);
        assertEq(hasProfit, false);
        assertEq(delta, 0);
    }

    function testShortsTrackerGetNextGlobalShortData() public {
        (uint256 nextSize, uint256 nextAveragePrice) = shortsTracker.getNextGlobalShortData(
            OWNER, weth, weth, 2000000000000000000000000000000000, 35689554309499200000000000000000000, true
        );
        assertNotEq(nextSize, 0);
        assertNotEq(nextAveragePrice, 0);
    }

    function testShortsTrackerGetRealisedPnl() public {
        int256 realisedPnl = shortsTracker.getRealisedPnl(OWNER, weth, weth, 35689554309499200000000000000000000, true);
        assertEq(realisedPnl, 0);
    }

    function testShortsTrackerGetNextGlobalAveragePrice() public {
        uint256 nextAveragePrice = shortsTracker._getNextGlobalAveragePrice(
            2000000000000000000000000000000000, 2000000000000000000000000000000000, 1, 1, 1
        );
        assertNotEq(nextAveragePrice, 0);
    }
}
