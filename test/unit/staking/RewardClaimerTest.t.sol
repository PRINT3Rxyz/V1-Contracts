...
contract RewardClaimerTest is Test {
    ...
    function setUp() public {
        ...
        rewardClaimer = contracts.staking.rewardClaimer;
    ...
    function testRewardClaimerLetsUsersClaimAllPendingRewardsFromOneLocation() public giveUserCurrency {
        // stake some tokens
        vm.startPrank(USER);
        WETH(weth).approve(address(brrrManager), LARGE_AMOUNT);
        uint256 brrrAmount = rewardRouter.mintAndStakeBrrr(weth, 5e18, 1, 1);
        transferStakedBrrr.approve(address(amplifier), brrrAmount);
        // lock some tokens
        amplifier.lockLiquidity(3, brrrAmount / 2);
        // wait some time
        vm.warp(block.timestamp + 7 days);
        vm.roll(block.number + 1);
        // claim
        (uint256 wethRewards, uint256 xpRewards) = rewardClaimer.claimAllPendingRewards();
        vm.stopPrank();
        assertGt(wethRewards, 0);
        assertGt(xpRewards, 0);
    }
}
