...
contract RewardDistributorTest is Test {
    ...
    // Setter Tests //
    //////////////////
    function testRewardDistributorSetAdmin() public {
        vm.prank(OWNER);
        rewardDistributor.setAdmin(USER);
        assertEq(rewardDistributor.admin(), USER);
    }

    function testRewardDistributorSetTokensPerInterval(uint256 _val) public {
        vm.prank(OWNER);
        rewardDistributor.setTokensPerInterval(_val);
        assertEq(rewardDistributor.tokensPerInterval(), _val);
    }
    //////////////////
    // Getter Tests //
    //////////////////

    function testRewardDistributorGetPendingRewards() public {
        vm.prank(OWNER);
        rewardDistributor.pendingRewards();
    }
    ////////////////////
    // Function Tests //
    ////////////////////

    function testRewardDistributorLetsUsersRecoverWronglySentTokens() public {
        vm.startPrank(OWNER);
        WBTC(wbtc).transfer(address(rewardDistributor), SMALL_AMOUNT);
        rewardDistributor.withdrawToken(wbtc, OWNER, SMALL_AMOUNT);
        vm.stopPrank();
        assertEq(WBTC(wbtc).balanceOf(address(rewardDistributor)), 0);
    }

    function testRewardDistributorUpdateLastDistributionTime() public {
        vm.prank(OWNER);
        rewardDistributor.updateLastDistributionTime();
    }

    function testRewardDistributorDistribute() public returns (uint256, uint256, uint256) {
        vm.prank(OWNER);
        WETH(weth).transfer(address(rewardDistributor), 1e24);
        assertGt(rewardDistributor.lastDistributionTime(), 0);
        ...
        assertGt(rewardDistributor.pendingRewards(), 0);
        vm.prank(address(rewardTracker));
        uint256 amount2 = rewardDistributor.distribute();
        assertNotEq(amount2, 0);
        vm.warp(block.timestamp + 1 days);
        vm.roll(block.number + 1);
        assertGt(rewardDistributor.pendingRewards(), 0);
        vm.prank(address(rewardTracker));
        uint256 amount3 = rewardDistributor.distribute();
        assertNotEq(amount3, 0);
