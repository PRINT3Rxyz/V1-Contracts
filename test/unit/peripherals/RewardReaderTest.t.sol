...
contract RewardReaderTest is Test {
    ...
    function testRewardReaderGetDepositBalances() public returns (uint256[] memory amounts) {
        brrrArray.push(address(brrr));
        rewardArray.push(address(rewardTracker));
        amounts = rewardReader.getDepositBalances(OWNER, brrrArray, rewardArray);
    }

    function testRewardReaderGetStakingInfo() public returns (uint256[] memory amounts) {
        rewardArray.push(address(rewardTracker));
        amounts = rewardReader.getStakingInfo(OWNER, rewardArray);
    }
}
