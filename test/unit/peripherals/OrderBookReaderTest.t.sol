...
contract OrderBookReaderTest is Test {
    ...
    function testOrderBookReaderCanGetIncreaseOrders()
        public
        giveUserCurrency
    ...
    function testOrderBookReaderCanGetDecreaseOrders()
        public
        giveUserCurrency
        returns (uint256[] memory uintProps, address[] memory addressProps)
    {
        ...
        orderBook.createIncreaseOrder{value: 300000000000000}(
            ...
            weth,
            true,
            1961587547909500000000000000000000,
            true,
            300000000000000,
            false
        );
        ...
        // Execute the order
        vm.prank(OWNER);
        positionManager.executeIncreaseOrder(USER, 0, payable(OWNER));
        // Create a decrease order
        vm.prank(USER);
        orderBook.createDecreaseOrder{value: 300000000000001}(
            weth, 1e17, weth, 0, true, 2061587547909500000000000000000000, true
        );
        // Get the order
        (uintProps, addressProps) = orderBookReader.getDecreaseOrders(payable(address(orderBook)), USER, _indices);
    }

    function testOrderBookReaderCanGetSwapOrders() public giveUserCurrency {
        // Create a swap order
        vm.startPrank(USER);
        ...
        router.approvePlugin(address(orderBook));
        WETH(weth).increaseAllowance(address(router), LARGE_AMOUNT);
        orderBook.createSwapOrder{value: 300000000000000}(_path, 1e18, 1, 1, true, 300000000000000, false, false);
        vm.stopPrank();
        // Check the limit order is there
        numberArray.push(0);
        uint256[] memory _indices = numberArray;
        orderBookReader.getSwapOrders(payable(address(orderBook)), USER, _indices);
    }
}
