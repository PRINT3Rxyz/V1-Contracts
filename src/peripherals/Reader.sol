...
contract Reader is Governable {
    ...
    function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
        ...
        uint256 maxUsdpAmount = _vault.maxUsdpAmounts(_tokenIn);

        if (maxUsdpAmount != 0) {
            if (maxUsdpAmount < _vault.usdpAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdpAmount - _vault.usdpAmounts(_tokenIn);
            maxAmountIn = (maxAmountIn * (10 ** tokenInDecimals)) / (10 ** USDP_DECIMALS);
        ...
        returns (uint256, uint256)
    {
        ...
        {
            uint256 usdpAmount = (_amountIn * priceIn) / PRICE_PRECISION;
            usdpAmount = (usdpAmount * (10 ** USDP_DECIMALS)) / (10 ** tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usdpAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usdpAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _usdpAmount, address[] memory _tokens)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.usdpAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _usdpAmount);
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _usdpAmount, address[] memory _tokens)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxUsdpAmounts(token);
    ...
    function getVaultTokenInfoV2(address _vault, address _weth, uint256 _usdpAmount, address[] memory _tokens)
        public
        view
        returns (uint256[] memory)
    {
