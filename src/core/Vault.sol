...
contract Vault is ReentrancyGuard, IVault {
    ...
    // usdpAmounts tracks the amount of USDP debt for each whitelisted token
    mapping(address => uint256) public override usdpAmounts;

    // maxUsdpAmounts allows setting a max amount of USDP debt for a token
    mapping(address => uint256) public override maxUsdpAmounts;
    ...
    // this value is used to calculate the redemption values for selling of USDP
    ...
    event BuyUSDP(address account, address token, uint256 tokenAmount, uint256 usdpAmount, uint256 feeBasisPoints);
    event SellUSDP(address account, address token, uint256 usdpAmount, uint256 tokenAmount, uint256 feeBasisPoints);
    event Swap(
        address account,
        address tokenIn,
        address tokenOut,
    ...
    function setTokenConfig(
        ...
        maxUsdpAmounts[_token] = _maxUsdpAmount;
    ...
    function setUsdpAmount(address _token, uint256 _amount) external override {
        _onlyGov();

        uint256 usdpAmount = usdpAmounts[_token];
        if (_amount > usdpAmount) {
            _increaseUsdpAmount(_token, _amount - usdpAmount);
            return;
        }

        _decreaseUsdpAmount(_token, usdpAmount - _amount);
    }
    ...
    function buyUSDP(address _token, address _receiver) external override nonReentrant returns (uint256) {
        ...
        uint256 usdpAmount = (tokenAmount * price) / PRICE_PRECISION;
        usdpAmount = adjustForDecimals(usdpAmount, _token, usdp);
        _validate(usdpAmount > 0, 18);

        uint256 feeBasisPoints = vaultUtils.getBuyUsdpFeeBasisPoints(_token, usdpAmount);
        ...
        emit BuyUSDP(_receiver, _token, tokenAmount, mintAmount, feeBasisPoints);

        useSwapPricing = false;
        return mintAmount;
    }

    function sellUSDP(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        _validate(whitelistedTokens[_token], 19);
        useSwapPricing = true;

        uint256 usdpAmount = _transferIn(usdp);
        _validate(usdpAmount > 0, 20);
        ...
        uint256 feeBasisPoints = vaultUtils.getSellUsdpFeeBasisPoints(_token, usdpAmount);
        ...
        returns (uint256)
    {
        ...
        // adjust usdpAmounts by the same usdpAmount as debt is shifted between the assets
        uint256 usdpAmount = (amountIn * priceIn) / PRICE_PRECISION;
        usdpAmount = adjustForDecimals(usdpAmount, _tokenIn, usdp);

        uint256 feeBasisPoints = vaultUtils.getSwapFeeBasisPoints(_tokenIn, _tokenOut, usdpAmount);
        uint256 amountOutAfterFees = _collectSwapFees(_tokenOut, amountOut, feeBasisPoints);

        _increaseUsdpAmount(_tokenIn, usdpAmount);
        _decreaseUsdpAmount(_tokenOut, usdpAmount);
    ...
    function getRedemptionAmount(address _token, uint256 _usdpAmount) public view override returns (uint256) {
    ...
    function adjustForDecimals(uint256 _amount, address _tokenDiv, address _tokenMul) public view returns (uint256) {
        uint256 decimalsDiv = _tokenDiv == usdp ? USDP_DECIMALS : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == usdp ? USDP_DECIMALS : tokenDecimals[_tokenMul];
    ...
    function getFeeBasisPoints(
        ...
        return vaultUtils.getFeeBasisPoints(_token, _usdpDelta, _feeBasisPoints, _taxBasisPoints, _increment);
    }

    function getTargetUsdpAmount(address _token) public view override returns (uint256) {
    ...
    function _increaseUsdpAmount(address _token, uint256 _amount) private {
        usdpAmounts[_token] = usdpAmounts[_token] + _amount;
        uint256 maxUsdpAmount = maxUsdpAmounts[_token];
        if (maxUsdpAmount != 0) {
    ...
    function _decreaseUsdpAmount(address _token, uint256 _amount) private {
        uint256 value = usdpAmounts[_token];
