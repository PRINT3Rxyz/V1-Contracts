...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _weth, _usdp, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }
    ...
    function createSwapOrder(
        ...
        _createSwapOrder(
            msg.sender, _path, _amountIn, _minOut, _triggerRatio, _triggerAboveThreshold, _shouldUnwrap, _executionFee
        );
    }
    ...
    function _createSwapOrder(
        ...
        SwapOrder memory order = SwapOrder(
            _account, _path, _amountIn, _minOut, _triggerRatio, _triggerAboveThreshold, _shouldUnwrap, _executionFee
        );
    ...
    function getUsdpMinPrice(address _otherToken) public view returns (uint256) {
        // USDP_PRECISION is the same as 1 USDP
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, USDP_PRECISION);
        ...
        returns (bool)
    {
        ...
        // 1. USDP doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why USDP price in USD can vary depending on the redepmtion token
        // 2. In complex scenarios with path=[USDP, BNB, BTC] we need to know how much BNB we'll get for provided USDP
        // to know how much BTC will be received
        // That's why in such scenario BNB should be used to determine price of USDP
        if (tokenA == usdp) {
            // with both _path.length == 2 or 3 we need usdp price against _path[1]
            tokenAPrice = getUsdpMinPrice(_path[1]);
        } else {
        ...
        returns (uint256)
    {
        ...
        if (_tokenOut == usdp) {
            // buyUSDP
            amountOut = IVault(vault).buyUSDP(_tokenIn, _receiver);
        } else if (_tokenIn == usdp) {
            // sellUSDP
            amountOut = IVault(vault).sellUSDP(_tokenOut, _receiver);
        } else {
            // swap
